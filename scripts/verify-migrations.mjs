...
const files = allSqlFiles.filter(name => /^\d+_.+\.sql$/.test(name)).sort((left, right) => Number(left.match(/^(\d+)_/)?.[1]) - Number(right.match(/^(\d+)_/)?.[1]) || left.localeCompare(right));
...
const requiredLifecycleGuards = [/create\s+or\s+replace\s+function\s+public\.validate_hiring_state_tenant/i,/language\s+plpgsql/i,/security\s+definer/i,/set\s+search_path\s*=\s*public/i,/create\s+trigger\s+hiring_state_history_tenant_guard/i,/before\s+insert\s+or\s+update\s+on\s+public\.hiring_state_history/i,/revoke\s+all\s+on\s+public\.hiring_state_history\s+from\s+anon\s*,\s*authenticated/i,/candidate_workflow\s*<>\s*new\.workflow_id/i];
for (const pattern of requiredLifecycleGuards) if (!pattern.test(lifecycleMigration)) throw new Error(`Hiring lifecycle persistence guard missing: ${pattern}`);

const rlsMigration = await readFile(path.join(root, '014_recruiting_core_rls_force.sql'), 'utf8');
for (const table of ['hiring_workflows', 'recruiting_candidates', 'recruiting_interviews']) {
  ...
  if (!tablePattern.test(rlsMigration) || !forcePattern.test(rlsMigration) || !revokePattern.test(rlsMigration)) throw new Error(`Recruiting core RLS coverage incomplete for ${table}`);
}
...
for (const table of ['recruiting_audit_events', 'hiring_state_history']) {
  ...
  if (!matches.includes('enable') || !matches.includes('force')) throw new Error(`Recruiting audit RLS coverage incomplete for ${table}`);
}

const tenantIntegrityMigration = await readFile(path.join(root, '015_recruiting_tenant_integrity_fks.sql'), 'utf8');
const requiredTenantIntegrityConstraints = [/create\s+unique\s+index\s+if\s+not\s+exists\s+hiring_workflows_tenant_id_uidx/i,/create\s+unique\s+index\s+if\s+not\s+exists\s+recruiting_candidates_tenant_id_uidx/i,/recruiting_candidates_tenant_workflow_fk[\s\S]*foreign\s+key\s*\(tenant_id\s*,\s*workflow_id\)[\s\S]*references\s+public\.hiring_workflows\s*\(tenant_id\s*,\s*id\)[\s\S]*on\s+delete\s+cascade[\s\S]*not\s+valid/i,/recruiting_audit_events_tenant_workflow_fk[\s\S]*foreign\s+key\s*\(tenant_id\s*,\s*workflow_id\)[\s\S]*references\s+public\.hiring_workflows\s*\(tenant_id\s*,\s*id\)[\s\S]*on\s+delete\s+cascade[\s\S]*not\s+valid/i,/recruiting_audit_events_tenant_candidate_fk[\s\S]*foreign\s+key\s*\(tenant_id\s*,\s*candidate_id\)[\s\S]*references\s+public\.recruiting_candidates\s*\(tenant_id\s*,\s*id\)[\s\S]*on\s+delete\s+set\s+null[\s\S]*not\s+valid/i,/recruiting_interviews_tenant_workflow_fk[\s\S]*foreign\s+key\s*\(tenant_id\s*,\s*workflow_id\)[\s\S]*references\s+public\.hiring_workflows\s*\(tenant_id\s*,\s*id\)[\s\S]*on\s+delete\s+cascade[\s\S]*not\s+valid/i,/recruiting_interviews_tenant_candidate_fk[\s\S]*foreign\s+key\s*\(tenant_id\s*,\s*candidate_id\)[\s\S]*references\s+public\.recruiting_candidates\s*\(tenant_id\s*,\s*id\)[\s\S]*on\s+delete\s+cascade[\s\S]*not\s+valid/i];
for (const pattern of requiredTenantIntegrityConstraints) if (!pattern.test(tenantIntegrityMigration)) throw new Error(`Recruiting tenant integrity guard missing: ${pattern}`);
...
if (tenantConstraintNames.length !== 5 || new Set(tenantConstraintNames).size !== 5) throw new Error(`Expected five unique recruiting tenant FK constraints, found ${tenantConstraintNames.length}`);

const tenantIndexMigration = await readFile(path.join(root, '016_recruiting_tenant_fk_indexes.sql'), 'utf8');
const requiredTenantIndexes = ['recruiting_candidates_tenant_workflow_fk_idx','recruiting_audit_events_tenant_workflow_fk_idx','recruiting_audit_events_tenant_candidate_fk_idx','recruiting_interviews_tenant_workflow_fk_idx','recruiting_interviews_tenant_candidate_fk_idx'];
...
const requiredActorConstraints = [/alter\s+table\s+recruiting_approvals[\s\S]*add\s+constraint\s+recruiting_approvals_requested_by_nonblank_check/i,/check\s*\(length\(btrim\(requested_by\)\)\s+between\s+1\s+and\s+256\)/i,/alter\s+table\s+recruiting_audit_events[\s\S]*add\s+constraint\s+recruiting_audit_events_actor_nonblank_check/i,/check\s*\(length\(btrim\(actor\)\)\s+between\s+1\s+and\s+256\)/i];
for (const pattern of requiredActorConstraints) if (!pattern.test(actorConstraintMigration)) throw new Error(`Control-plane actor persistence guard missing: ${pattern}`);
...
for (const table of requiredIntegrationTables) {
  ...
  if (!enablePattern.test(integrationMigration) || !revokePattern.test(integrationMigration)) throw new Error(`Production recruiting integration RLS/revoke coverage incomplete for ${table}`);
}
...
const requiredIntegrationTenantGuards = [
  ...
  /recruiting_knockout_results_tenant_job_fk[\s\S]*foreign\s+key\s*\(tenant_id\s*,\s*job_id\)[\s\S]*references\s+hiring_workflows\s*\(tenant_id\s*,\s*id\)[\s\S]*on\s+delete\s+cascade[\s\S]*not\s+valid/i,
