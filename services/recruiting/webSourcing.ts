...
export async function searchWebCandidates(apiKey: string, role: any, limit = 8): Promise<WebCandidate[]> {
  ...
  return (Array.isArray(parsed?.candidates) ? parsed.candidates : [])
    .map((c: any) => ({ ...c, profileUrl: String(c?.profileUrl || '').trim(), source: String(c?.source || '').trim() || hostname(String(c?.profileUrl || '')), evidence: Array.isArray(c?.evidence) ? c.evidence.map((x: any) => String(x).trim()).filter(Boolean) : [] }))
