...
router.post('/offer/draft', async (req, res) => { try { const jobId = String(req.body?.jobId || ''); const candidateId = String(req.body?.candidateId || ''); if (!jobId) return res.status(400).json({ error: 'jobId is required' }); if (!candidateId) return res.status(400).json({ error: 'candidateId is required' }); await requireApproval(req, jobId, 'compensation'); const payload = createOffer(req.body); const saved = await saveHiringState(tenantId(req), jobId, 'offer', payload, candidateId); const approval = await createGate(req, jobId, 'offer', 'Review the offer package before it can be sent to the candidate.'); res.json({ ...payload, state: saved, approval }); } catch (error: any) { res.status(400).json({ error: error?.message || 'Offer drafting failed' }); } });
