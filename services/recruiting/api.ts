...
async function getCredential(req: any): Promise<SessionCredential> {
  ...
  const provider: AIProvider = process.env.GEMINI_API_KEY ? 'gemini' : (await listAIProviders(tenant))[0];
  if (provider) {
    const apiKey = await getAICredential(tenant, provider);
    if (apiKey) {
      const credential = { provider, apiKey, model: provider === 'gemini' ? 'gemini-3.6-flash' : undefined };
  ...
  if (process.env.GEMINI_API_KEY) {
    const credential = { provider: 'gemini' as AIProvider, apiKey: process.env.GEMINI_API_KEY, model: 'gemini-3.6-flash' };
    sessions.set(tenant, credential);
    return credential;
  }
...
async function validateCredential(provider: AIProvider, apiKey: string, model?: string) {
  await generateAI({
    provider,
    apiKey,
    model: model || (provider === 'gemini' ? 'gemini-3.6-flash' : undefined),
...
router.post('/ai/connect', async (req, res) => {
  try {
    ...
    const tenant = tenantId(req);
    const credential: SessionCredential = { provider: provider as AIProvider, apiKey: secret, model: selectedModel };
    sessions.set(tenant, credential);

    let persistent = false;
    try {
      await saveAICredential(tenant, provider as AIProvider, secret);
      persistent = true;
    } catch (persistenceError: any) {
      // A missing optional persistence configuration must not block the current recruiting session.
...
router.delete('/ai/disconnect', async (req, res) => {
  ...
  if (session) {
    try { await deleteAICredential(tenant, session.provider); } catch (error: any) { console.warn('AI credential delete unavailable:', error?.message || error); }
  }
...
router.post('/decision', (req, res) => {
  try { res.json(makeHiringDecision(req.body)); } catch (error: any) { res.status(400).json({ error: error?.message || 'Decision failed' }); }
});
router.post('/compensation/recommend', (req, res) => {
  try { res.json(recommendCompensation(req.body?.observations || [], req.body?.internalComparable)); } catch (error: any) { res.status(400).json({ error: error?.message || 'Compensation analysis failed' }); }
});
router.post('/offer/draft', (req, res) => {
  try { res.json(createOffer(req.body)); } catch (error: any) { res.status(400).json({ error: error?.message || 'Offer draft failed' }); }
});
