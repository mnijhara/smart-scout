...
router.post('/jd/analyze', async (req, res) => { try { const { text } = req.body || {}; if (!text?.trim()) return res.status(400).json({ error: 'Job description text is required' }); const c = await getCredential(req); const analysis = await analyzeJD(text, c.provider, c.apiKey, c.model); const job = await saveJob(tenantId(req), text.trim(), analysis); res.json({ ...analysis, jobId: job.id, savedAt: job.createdAt }); } catch (error: any) { res.status(400).json({ error: error?.message || 'JD analysis failed' }); } });
...
router.post('/source/search', async (req, res) => { try { const c = await getCredential(req); if (c.provider !== 'gemini') return res.status(400).json({ error: 'Candidate web sourcing currently requires Gemini' }); res.json({ candidates: await searchWebCandidates(c.apiKey, req.body?.role, Number(req.body?.limit) || 8) }); } catch (error: any) { res.status(400).json({ error: error?.message || 'Candidate sourcing failed' }); } });
