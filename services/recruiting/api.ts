...
router.get('/jobs/:id/approvals', async (req, res) => { try { res.json({ approvals: await listApprovals(tenantId(req), String(req.params.id)) }); } catch (error: any) { res.status(500).json({ error: error?.message || 'Unable to list approvals' }); } });
router.post('/jobs/:id/approvals/:approvalId/approve', async (req, res) => { try { const tenant = tenantId(req); const approvals = await listApprovals(tenant, String(req.params.id)); const current = approvals.find(x => x.id === req.params.approvalId); if (!current) return res.status(404).json({ error: 'Approval not found' }); const updated = await requestApproval({ tenantId: tenant, jobId: String(req.params.id), action: current.action, requestedBy: 'recruiter', note: `Approved by ${String(req.body?.approvedBy || 'recruiter')}` }); res.json({ approval: { ...updated, status: 'approved' } }); } catch (error: any) { res.status(400).json({ error: error?.message || 'Unable to approve request' }); } });
