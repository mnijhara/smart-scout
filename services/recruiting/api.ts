...
async function getCredential(req: any): Promise<SessionCredential> {
  ...
  if (provider) {
    ...
    if (apiKey) { const c = { provider, apiKey, model: provider === 'gemini' ? 'gemini-3.6-flash' : undefined }; sessions.set(tenant, c); return c; }
  }
...
router.post('/ai/connect', async (req, res) => {
  try { const { provider, apiKey, model } = req.body || {}; if (!['gemini','openai','anthropic'].includes(provider)) return res.status(400).json({ error: 'Unsupported provider' }); if (!String(apiKey || '').trim()) return res.status(400).json({ error: 'API key is required' }); await generateAI({ provider, apiKey: String(apiKey).trim(), model: model || (provider === 'gemini' ? 'gemini-3.6-flash' : undefined), system: 'Reply with OK only.', prompt: 'OK', temperature: 0, maxTokens: 8 }); const tenant = tenantId(req); sessions.set(tenant, { provider, apiKey: String(apiKey).trim(), model: model || (provider === 'gemini' ? 'gemini-3.6-flash' : undefined) }); try { await saveAICredential(tenant, provider, String(apiKey).trim()); } catch {} res.json({ connected: true, provider, model: model || null }); }
...
router.post('/jd/analyze', async (req, res) => { try { const c = await getCredential(req); const analysis = await analyzeJD(String(req.body?.text || ''), c.provider, c.apiKey, c.model); const job = await createJob(tenantId(req), { prompt: String(req.body?.text || ''), ...analysis }); res.json({ ...analysis, jobId: job.id, job }); } catch (error: any) { res.status(400).json({ error: error?.message || 'JD analysis failed' }); } });
