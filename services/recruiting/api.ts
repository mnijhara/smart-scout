...
router.post('/decision', async (req, res) => { try { const jobId = String(req.body?.jobId || ''); const candidateId = String(req.body?.candidateId || ''); if (!jobId || !candidateId) return res.status(400).json({ error: 'jobId and candidateId are required' }); await requireApproval(req, jobId, 'jd_approval'); const interviews = await listInterviews(tenantId(req), jobId); const completed = interviews.find(x => x.candidateId === candidateId && x.status === 'completed'); if (!completed) return res.status(409).json({ error: 'Complete the candidate interview before creating a hiring decision.' }); const payload = makeHiringDecision(req.body); const saved = await saveHiringState(tenantId(req), jobId, 'decision', payload, candidateId); const approval = await createGate(req, jobId, 'decision', 'Review the candidate evidence and approve the hiring recommendation before compensation.'); res.json({ ...payload, state: saved, approval }); } catch (error: any) { res.status(400).json({ error: error?.message || 'Decision failed' }); } });
router.post('/compensation/recommend', async (req, res) => { try { const jobId = String(req.body?.jobId || ''); if (!jobId) return res.status(400).json({ error: 'jobId is required' }); await requireApproval(req, jobId, 'decision'); const payload = recommendCompensation(req.body?.observations || [], req.body?.internalComparable); const saved = await saveHiringState(tenantId(req), jobId, 'compensation', payload, req.body?.candidateId); const approval = await createGate(req, jobId, 'compensation', 'Review the compensation recommendation before drafting an offer.'); res.json({ ...payload, state: saved, approval }); } catch (error: any) { res.status(400).json({ error: error?.message || 'Compensation analysis failed' }); } });
router.post('/offer/draft', async (req, res) => { try { const jobId = String(req.body?.jobId || ''); if (!jobId) return res.status(400).json({ error: 'jobId is required' }); await requireApproval(req, jobId, 'compensation'); const payload = createOffer(req.body); const saved = await saveHiringState(tenantId(req), jobId, 'offer', payload, req.body?.candidateId); const approval = await createGate(req, jobId, 'offer', 'Review the offer package before it can be sent to the candidate.'); res.json({ ...payload, state: saved, approval }); } catch (error: any) { res.status(400).json({ error: error?.message || 'Offer drafting failed' }); } });
router.post('/offer/transition', async (req, res) => { try { const jobId = String(req.body?.jobId || ''); const candidateId = req.body?.candidateId ? String(req.body.candidateId) : undefined; const next = String(req.body?.status || '') as any; if (!jobId) return res.status(400).json({ error: 'jobId is required' }); const states = await listHiringStates(tenantId(req), jobId, 'offer'); const latest = states[0]?.payload; if (!latest) return res.status(404).json({ error: 'Offer not found' }); if (next === 'approved' || next === 'sent') await requireApproval(req, jobId, 'offer'); if (next === 'sent' && latest.status !== 'approved') return res.status(409).json({ error: 'Approve the offer before sending it.' }); const payload = transitionOffer(latest, next); const saved = await saveHiringState(tenantId(req), jobId, 'offer', payload, candidateId); res.json({ ...payload, state: saved, approval: await latestApproval(tenantId(req), jobId, 'offer') }); } catch (error: any) { res.status(400).json({ error: error?.message || 'Offer transition failed' }); } });
...
router.post('/integrations/calendar/schedule', async (req, res) => { try { if (!process.env.CALENDAR_API_URL || !process.env.CALENDAR_API_TOKEN) return res.status(503).json({ error: 'Calendar integration is not configured' }); const payload = await postJson(process.env.CALENDAR_API_URL, process.env.CALENDAR_API_TOKEN, req.body); res.json({ ok: true, payload }); } catch (error: any) { res.status(502).json({ error: error?.message || 'Calendar provider request failed' }); } });
router.post('/integrations/transcription', async (req, res) => { try { if (!process.env.TRANSCRIPTION_API_URL || !process.env.TRANSCRIPTION_API_KEY) return res.status(503).json({ error: 'Transcription integration is not configured' }); const payload = await postJson(process.env.TRANSCRIPTION_API_URL, process.env.TRANSCRIPTION_API_KEY, req.body); res.json({ ok: true, payload }); } catch (error: any) { res.status(502).json({ error: error?.message || 'Transcription provider request failed' }); } });
router.post('/integrations/compensation/market-data', async (req, res) => { try { if (!process.env.COMPENSATION_API_URL || !process.env.COMPENSATION_API_KEY) return res.status(503).json({ error: 'Compensation market-data integration is not configured' }); const payload = await postJson(process.env.COMPENSATION_API_URL, process.env.COMPENSATION_API_KEY, req.body); res.json({ ok: true, payload }); } catch (error: any) { res.status(502).json({ error: error?.message || 'Compensation provider request failed' }); } });
router.post('/integrations/hris/employee', async (req, res) => { try { await requireApproval(req, String(req.body?.jobId || ''), 'employee_create'); if (!process.env.HRIS_API_URL || !process.env.HRIS_API_TOKEN) return res.status(503).json({ error: 'HRIS integration is not configured' }); const payload = await postJson(process.env.HRIS_API_URL, process.env.HRIS_API_TOKEN, req.body); res.json({ ok: true, payload }); } catch (error: any) { res.status(502).json({ error: error?.message || 'HRIS provider request failed' }); } });
