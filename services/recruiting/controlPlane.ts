...
async function append<T>(name: string, value: T): Promise<T> { await fs.mkdir(root,{recursive:true}); queues[name]=(queues[name]||Promise.resolve()).then(async()=>{const all=await read<T>(name);all.unshift(value);await fs.writeFile(path.join(root,name),JSON.stringify(all.slice(0,10000),null,2),'utf8')}); await queues[name]; return value; }
const now=()=>new Date().toISOString();

export async function requestApproval(input: Omit<Approval, keyof RecordBase>): Promise<Approval> { const t=now(); return append(files.approvals,{...input,id:`approval_${crypto.randomUUID()}`,createdAt:t,updatedAt:t,status:'pending'} as Approval); }
...
export async function audit(input: Omit<AuditEvent, keyof RecordBase>):Promise<AuditEvent>{const t=now();return append(files.audit,{...input,id:`audit_${crypto.randomUUID()}`,createdAt:t,updatedAt:t} as AuditEvent);}
...
export async function recordUsage(input: Omit<UsageRecord, keyof RecordBase>):Promise<UsageRecord>{const t=now();return append(files.usage,{...input,id:`usage_${crypto.randomUUID()}`,createdAt:t,updatedAt:t} as UsageRecord);}
