...
export async function listCandidates(tenantId:string,jobId:string):Promise<SavedCandidate[]>{tenantId=requireTenantId(tenantId);const client=db();if(client){const {data,error}=await client.from('recruiting_candidates').select('*').eq('tenant_id',tenantId).eq('workflow_id',workflowUuid(jobId)).order('updated_at',{ascending:false});if(error)throw new Error(`Unable to list candidates: ${error.message}`);return(data||[]).map(publicCandidate)}return(await readAll()).filter(x=>x.tenantId===tenantId&&x.jobId===jobId)}
export async function updateCandidateScore(tenantId:string,id:string,score:any):Promise<SavedCandidate|null>{tenantId=requireTenantId(tenantId);const client=db();if(client){const candidateId=id.startsWith('candidate_')?id.slice(10):id;const {data,error}=await client.from('recruiting_candidates').update({score,updated_at:new Date().toISOString()}).eq('tenant_id',tenantId).eq('id',candidateId).select('*').maybeSingle();if(error)throw new Error(`Unable to update candidate score: ${error.message}`);return data?publicCandidate(data):null}let result:SavedCandidate|null=null;writeQueue=writeQueue.then(async()=>{const all=await readAll(),index=all.findIndex(x=>x.tenantId===tenantId&&x.id===id);if(index<0){result=null;return}result={...all[index],score,updatedAt:new Date().toISOString()};all[index]=result;await fs.writeFile(filePath,JSON.stringify(all,null,2),'utf8')});await writeQueue;return result}
