...
export default function AuthGateRelease({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{let mounted=true;let unsubscribe=()=>{};(async()=>{try{
   ...
   if(mounted&&!session.ok)setNotice('Sign in with Google to save hiring work to your account.');
   if(mounted)setWorkspaceReady(true);
 }catch(e:any){if(mounted){setNotice(e?.message||'Sign in to continue.');setWorkspaceReady(true)}}finally{if(mounted)setLoading(false)}})();return()=>{mounted=false;unsubscribe()}},[]);
