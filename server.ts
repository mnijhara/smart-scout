...
import recruitingRouter from './services/recruiting/api';
...
async function startServer() {
  ...
  app.post('/api/send-report', async (req, res) => {
    ...
    try {
      ...
      doc.setFontSize(14); doc.text(`Candidate: ${candidateName}`, 20, 35);
      doc.text(`Overall Score: ${overallScore}%`, 20, 45); doc.text(`Status: ${status}`, 20, 55);
      doc.setFontSize(16); doc.text('Executive Summary', 20, 70);
      doc.setFontSize(12);
      const splitReason = doc.splitTextToSize(String(reason || ''), 170); doc.text(splitReason, 20, 80);
      let y = 80 + splitReason.length * 7;
      doc.setFontSize(16); doc.text('Score Breakdown', 20, y + 10); doc.setFontSize(12); y += 20;
      parameters.forEach((p: any) => { doc.text(`${p.name}: ${p.score}%`, 20, y); y += 10; });
      doc.setFontSize(16); doc.text('Q&A Transcript', 20, y + 10); doc.setFontSize(12); y += 20;
      responses.forEach((r: any, index: number) => {
        const q = doc.splitTextToSize(`Q${index + 1}: ${r.question}`, 170); doc.text(q, 20, y); y += q.length * 7;
        const a = doc.splitTextToSize(`A: ${r.answer}`, 170); doc.text(a, 20, y); y += a.length * 7 + 5;
      });
      const pdfBuffer = Buffer.from(doc.output('arraybuffer'));
      const { data, error } = await resend.emails.send({
        from: 'SmartScout <reports@smartscout.online>', to: [recruiterEmail],
  ...
  app.post('/api/send-invitation', async (req, res) => {
    ...
    try {
      ...
      if (scheduledAt) {
        ...
        if (!error && value) attachments.push({ filename: 'interview-invite.ics', content: Buffer.from(value) });
      }
      const { data, error } = await resend.emails.send({
        from: 'SmartScout <interviews@smartscout.online>', to: [candidateEmail],
