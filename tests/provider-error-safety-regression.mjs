...
assert.match(auth, /if\(token\.length > MAX_FIREBASE_TOKEN_LENGTH\)throw new Error\('Invalid Firebase authentication token'\);/, 'oversized Firebase tokens must be rejected locally');
...
console.log('Authentication provider failures are safe and oversized Firebase tokens are rejected before provider lookup.');
