-- Keep database-side lifecycle input limits aligned with the service boundary.
-- The privileged RPC must fail closed even when called without the application layer.
...
begin
  ...
  if length(btrim(p_tenant_id)) > 256 then
    raise exception 'hiring state tenant exceeds 256 characters';
  end if;
  if nullif(btrim(p_state_type), '') is null then
    raise exception 'hiring state type is required';
  end if;
  if length(btrim(p_state_type)) > 128 then
    raise exception 'hiring state type exceeds 128 characters';
  end if;
  ...
  if octet_length(convert_to(coalesce(p_payload, '{}'::jsonb)::text, 'UTF8')) > 65536 then
    raise exception 'hiring state payload exceeds 65536 bytes';
  end if;
...
comment on function public.persist_hiring_state_with_audit(text, uuid, uuid, text, jsonb, text) is
  'Atomically persists a bounded hiring lifecycle state and its corresponding audit event; failure rolls back both inserts.';
