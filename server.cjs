...
app.post('/api/send-report', async (req, res) => {
  try {
    ...
    const result = await resend.emails.send({ from: 'SmartScout <reports@smartscout.online>', to: [recruiterEmail], subject: `Interview Report: ${candidateName} (${status} - ${overallScore}%)`, attachments: [{ filename: `${String(candidateName || 'candidate').replace(/\s+/g, '_')}_Report.pdf`, content: Buffer.from(doc.output('arraybuffer')) }], html: `<div style="font-family:sans-serif"><h1>Interview Report</h1><p><b>Candidate:</b> ${candidateName || ''}</p><p><b>Overall Score:</b> ${overallScore || ''}%</p><p><b>Status:</b> ${status || ''}</p><p>${reason || ''}</p></div>` });
...
if (!require('fs').existsSync(INDEX)) {
  console.error(`Hostinger startup error: ${INDEX} does not exist. The build command must be: npm run build`);
  process.exit(1);
}
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Smart Scout running on 0.0.0.0:${PORT}`);
});
