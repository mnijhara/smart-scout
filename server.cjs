...
// Health check for Hostinger/container probes.
app.get('/health', (_req, res) => {
  res.status(200).json({ ok: true, service: 'smartscout' });
});

// Stripe Checkout Session. Dependencies are loaded lazily so a missing optional
// integration cannot prevent the website itself from starting.
...
app.post('/api/send-report', async (req, res) => {
  try {
    ...
    doc.setFontSize(16); doc.text('Executive Summary', 20, 70);
    doc.setFontSize(12);
    const splitReason = doc.splitTextToSize(reason || '', 170);
    doc.text(splitReason, 20, 80);
    let y = 80 + splitReason.length * 7;
    doc.setFontSize(16); doc.text('Score Breakdown', 20, y + 10);
    doc.setFontSize(12); y += 20;
    ...
    responses.forEach((r, index) => {
      ...
      doc.text(q, 20, y); y += q.length * 7;
      const a = doc.splitTextToSize(`A: ${r.answer || ''}`, 170);
      doc.text(a, 20, y); y += a.length * 7 + 5;
    });
    ...
    const result = await resend.emails.send({
      ...
      attachments: [{ filename: `${String(candidateName || 'candidate').replace(/\s+/g, '_')}_Report.pdf`, content: pdfBuffer }],
      html: `<div style="font-family:sans-serif"><h1>Interview Report</h1><p><b>Candidate:</b> ${candidateName || ''}</p><p><b>Overall Score:</b> ${overallScore || ''}%</p><p><b>Status:</b> ${status || ''}</p><p>${reason || ''}</p></div>`,
    });
    if (result.error) return res.status(500).json({ success: false, error: result.error.message });
    res.json({ success: true, data: result.data });
  } catch (err) {
    console.error('Report error:', err);
    res.status(500).json({ success: false, error: err.message || 'Failed to send report' });
  }
});
  ...
  try {
    ...
    if (scheduledAt) {
      ...
      const event = {
        ...
        location: 'SmartScout AI Platform', url: interviewLink, status: 'CONFIRMED', busyStatus: 'BUSY',
      ...
      const created = ics.createEvent(event);
      if (!created.error && created.value) attachments.push({ filename: 'interview-invite.ics', content: Buffer.from(created.value) });
    }
    const result = await resend.emails.send({
      ...
      attachments,
      html: `<div style="font-family:sans-serif;max-width:600px"><h1>Interview Invitation</h1><div style="white-space:pre-wrap">${emailBody || ''}</div>${scheduledAt ? `<p>Scheduled Time: ${new Date(scheduledAt).toLocaleString()}</p>` : ''}<p>Best regards,<br>Smart Scout Recruitment Team</p></div>`,
    });
    if (result.error) return res.status(500).json({ success: false, error: result.error.message });
    res.json({ success: true, data: result.data });
  } catch (err) {
    console.error('Invitation error:', err);
    res.status(500).json({ success: false, error: err.message || 'Failed to send invitation' });
  }
});

// Serve the Vite production build. Keep SPA fallback so /?os=1 and future
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Smart Scout server running on 0.0.0.0:${PORT}`);
});
