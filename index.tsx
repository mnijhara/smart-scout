...
if (container) {
  ...
  const publicHome = (
    <>
      <LandingPageFinal onStart={() => window.location.assign('/hire')} />
      <MagicHiringDemoV3 onUseOwn={() => window.location.assign('/hire')} />
    </>
  );
  ...
  const lifecycle = (
    <AuthGate>
      <>
        <JDImportBridge />
        <HiringLifecycleV2 onBack={() => window.location.assign('/')} />
      </>
    </AuthGate>
  );
